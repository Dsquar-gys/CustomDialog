...
//                         if (!toClose) return;
//                         
//                         var multiple = content!.FileList.SelectedEntities;
//
//                         var result = multiple.Count == 1
//                             ? multiple[0].FullPath
//                             : Path.Combine(content.FileList.FilePath!, InitialFileName + "." + DefaultExtension);
//                     
//                         window.Close(result);
//                     });
//             })
//         };
//         
//         if (Filters is not null && Filters.Count > 0) mainWindowViewModel.FileDialogVM.Filters = Filters;
//         
//         // Command for dialog main view `Save` button
//         mainWindowViewModel.FileDialogVM.InvokeDialogAssignment = ReactiveCommand.Create(() =>
//         {
//             var body = mainWindowViewModel.FileDialogVM.FileList;
